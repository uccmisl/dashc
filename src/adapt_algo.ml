...
module Conv : sig
  ...
  (* The main difference in TAPAS tool implementation is conversion of the target rate to representation level,
     it also has buffer size of 60 seconds and only the first segment is downloaded in the lowest quality.
     This flag affects only conversion implementation.
     The code of the TAPAS tool could've been found here (https://github.com/ldecicco/tapas) during 08.2017.
  ...
  let next_representation_level ~representations ~results =
    if List.length results < 2 then 1
      ...
      (* Del_Rate *)
      (* The range of integers is -2^30 to 2^30-1 (or -2^62 to 2^62-1), so this conversion to Int64 is necessary for 32 bit platforms,
         Int32 and Int64 are both slower than ordinary int *)
      (* let throughput = float_of_int (last_result.segment_size * 8 * us * us) /. float_of_int (last_result.time_for_delivery) in *)
      (* https://github.com/janestreet/base/blob/master/src/int64.ml: let ( * ) = mul *)
      (*let throughput = Int64.to_float ( Int64.( * ) (Int64.( * ) (Int64.of_int last_result.segment_size) (Int64.of_int 8)) (Int64.( * ) (Int64.of_int us) (Int64.of_int us)) ) /. float_of_int (last_result.time_for_delivery) in*)
      (* OCaml's floating-point numbers follow the IEEE 754 standard, using double precision (64 bits) numbers *)
      let throughput = ((float_of_int last_result.segment_size) *. 8. *. us_float *. us_float) /. float_of_int (last_result.time_for_delivery) in
      moving_average :=
      ...
      if tapas_impl then
        ...
        let r_up = Hashtbl.fold representations ~init:1 ~f:(fun ~key ~data acc ->
          if (data.bandwidth < int_of_float (conv_weight *. !moving_average) &&
              data.bandwidth > (Hashtbl.find_exn representations acc).bandwidth) then key else acc) in
...
module BBA_1 = struct
  ...
  let calculate_reservoir ?(debug=false) ~algo ~representations ~results last_segment_index =
    ...
    let last_window_segment =
      if ((int_of_float algo.maxb) * 2) / segment_duration < last_segment_index - number_of_downloaded_segments then
    ...
    let rate_prev =
      ...
      (* when the representation rate is saved in a trace file, some level of precision will be lost,
      ...
      if debug then Hashtbl.fold representations ~init:1 ~f:(fun ~key ~data acc ->
        if (float_of_int data.bandwidth *. 0.95) < float_of_int (List.hd_exn results).representation_rate &&
           (float_of_int data.bandwidth *. 1.05) > float_of_int (List.hd_exn results).representation_rate
           then data.bandwidth
        else acc
      )
    ...
    let rec look_ahead_for_chunk_sizes ~curr_index (large_chunks_size, small_chunks_size) =
      (* check of equality to last_window_segment is made because List.nth_exn counts starting from 0 *)
      if curr_index = last_window_segment then (large_chunks_size, small_chunks_size)
        ...
        let segm_size = List.nth_exn (Hashtbl.find_exn algo.chunk_sizes_per_repr 1) curr_index in
        (* equal chunk_sizes are included to small category *)
        if average_chunk_size_per_min_rate < segm_size then
          look_ahead_for_chunk_sizes ~curr_index:(curr_index + 1) (large_chunks_size + segm_size, small_chunks_size)
        else
          look_ahead_for_chunk_sizes ~curr_index:(curr_index + 1) (large_chunks_size, small_chunks_size + segm_size)
    in
    let large_chunks_size, small_chunks_size = look_ahead_for_chunk_sizes ~curr_index:number_of_downloaded_segments (0, 0) in
...
(* the main difference from BBA_0 is usage of chunk map instead of rate map
  ...
  let next_representation_level ?(debug=false) ~algo ~representations ~results last_segment_index =
    let buf_now =
      if (List.length results) = 0 then 0.
      else (List.hd_exn results).buffer_level_in_momentum in
    ...
    let chunk_size_prev =
      if (List.length results) = 0 then
        ...
        List.nth_exn (Hashtbl.find_exn algo.chunk_sizes_per_repr repr_prev) (segm_number_next - 1)
    in

    (* update reservoir on each iteration *)
    let reservoir = float_of_int (calculate_reservoir ~debug:debug ~algo:algo ~representations ~results last_segment_index) in
    let maxbuf = algo.maxb in
    ...
    let rec get_chunk_sizes_per_segm_number ~curr_index ~chunk_sizes =
      if curr_index > (Hashtbl.length representations) then
        ...
        let chunk_size = List.nth_exn (Hashtbl.find_exn algo.chunk_sizes_per_repr curr_index) segm_number_next in
        get_chunk_sizes_per_segm_number ~curr_index:(curr_index + 1) ~chunk_sizes:(chunk_size :: chunk_sizes)
    in
    (* chunk_sizes_per_segm_number is a list of chunk sizes with the index as a representation number *)
    let chunk_sizes_per_segm_number = get_chunk_sizes_per_segm_number ~curr_index:1 ~chunk_sizes:[] in
    ...
    in
    let chunk_size_opt_discrete = List.fold chunk_sizes_per_segm_number ~init:(List.nth_exn chunk_sizes_per_segm_number 0) ~f:(fun acc x ->
      let () =
        if debug then begin
    ...
    let () =
      if debug then begin
        print_endline @@ "chunk_size_opt_discrete (BBA-1): " ^ string_of_int chunk_size_opt_discrete;
      end
    in

    (* next highest chunk size for the next segment *)
    let (chunk_size_plus, repr_plus) =
      if rate_prev = rate_max then List.nth_exn (Hashtbl.find_exn algo.chunk_sizes_per_repr repr_prev) segm_number_next, repr_prev
      else List.nth_exn (Hashtbl.find_exn algo.chunk_sizes_per_repr (repr_prev + 1)) segm_number_next, repr_prev + 1
    in
    let chunk_size_minus, repr_minus =
      if rate_prev = rate_min then List.nth_exn (Hashtbl.find_exn algo.chunk_sizes_per_repr 1) segm_number_next, repr_prev
      else List.nth_exn (Hashtbl.find_exn algo.chunk_sizes_per_repr (repr_prev - 1)) segm_number_next, repr_prev - 1
    in

    (* from ty-thesis, page 68: the algorithm stays at the
       current video rate as long as the chunk size suggested by the map does not pass the size of
       the next upcoming chunk at the next highest available video rate (Rate + ) or the next lowest
       available video rate (Rate ). *)
    (* the returned repr_next here begins from 0, but it should from 1, so it is increased later,
    ...
    let chunk_size_next, repr_next =
      ...
      if chunk_size_opt_discrete >= chunk_size_plus then
        List.foldi chunk_sizes_per_segm_number ~init:(chunk_size_plus, 0) ~f:(fun idx acc x ->
      ...
      (* if chunk_size_opt < chunk_size_plus && chunk_size_opt > chunk_size_minus then chunk_size_prev *)
...
(* The termination of the startup phase will happen in case of 1) the buffer is decreasing or 2) the chunk map suggests a higher rate.
   After that even if cushion is not full, the algorithms will be in steady-state all the time.
   This logic was approved by the author of the algorithm. *)
  let startup_phase = ref true

  let next_representation_level ?(debug=false) ~algo ~representations ~results last_segment_index =
    let open BBA_1 in
    let repr_next_bba_1 = BBA_1.next_representation_level ~debug:debug ~algo:algo ~representations:representations ~results:results last_segment_index in
    if List.length results = 0 then
      repr_next_bba_1
    else
      let buf_now =
        (* this condition was already checked above, but it will be checked again just in case this code maybe copy-pasted *)
        if (List.length results) = 0 then 0.
        else (List.hd_exn results).buffer_level_in_momentum
      in
      let segment_duration = ((Hashtbl.find_exn representations 1).segment_duration) in
      (* time_for_delivery is stored in us *)
      let prev_time_for_delivery = float_of_int (List.hd_exn results).time_for_delivery /. (us_float *. us_float) in
      (* positive delta_b means the buffer is increasing *)
      let delta_b = float_of_int segment_duration -. prev_time_for_delivery in
      let () =
        if debug then begin
          print_endline @@ "prev_time_for_delivery = " ^ string_of_float @@ float_of_int (List.hd_exn results).time_for_delivery /. (us_float *. us_float);
          print_endline @@ "delta_b = " ^ string_of_float delta_b;
        end
      in
      (* BBA-2 continues to use this startup algorithm until (1) the buffer is decreasing, or (2) the chunk map suggests a higher rate.
         If any of these conditions is false then switch to the steady-state forever. *)
      if delta_b >= 0. && !startup_phase then
        (* According to the BBA-2 paper, the bitrate increases only if the chunk is downloaded 8x (0.875 coefficient) faster than segment duration
           and this condition linearly decreases to 2x (0.5 coefficient) by the time cushion is full from the time when the first chunk was downloaded,
           so the coefficient can be calculated as a function of bitrate level.
           Calculation of the target_coefficient was approved by the author of the algorithm. *)
        let f buf =
          let slope = (0.5 -. 0.875) /. (0.9 *. algo.maxb -. float_of_int segment_duration) in
          let target_coefficient = 0.875 +. slope *. buf in
          let () =
            if debug then begin
              print_endline @@ "target_coefficient = " ^ string_of_float target_coefficient;
            end
          in
          target_coefficient
        in
        ...
        let suggested_repr =
          let () =
            if debug then begin
              print_endline @@ "target_coefficient *. (float_of_int segment_duration) = " ^ string_of_float @@ target_coefficient *. (float_of_int segment_duration);
            end
          in
          if delta_b > target_coefficient *. (float_of_int segment_duration) then
            let () =
              if debug then begin
                print_endline @@ "repr increase based on delta_b > target_coefficient *. (float_of_int segment_duration)";
              end
            in
            repr_plus
          else
        ...
        if repr_next_bba_1 <= suggested_repr then
          ...
          let () =
            if debug then begin
              print_endline @@ "repr_next_bba_1 (just switched to BBA-1): " ^ string_of_int repr_next_bba_1;
            end
          in
          startup_phase := false;
          repr_next_bba_1
    ...
    if List.length results < 2 then 1
      ...
      let rec calculate_weights ~curr_index ~acc_weights =
        if curr_index >= window_size then
          ...
          let numerator = exponential_weight *. (1. -. exponential_weight) ** float_of_int curr_index in
          let denominator = 1. -. (1. -. exponential_weight) ** float_of_int estimation_window in
          let () =
            if debug then begin
          ...
          in
          calculate_weights ~curr_index:(curr_index + 1) ~acc_weights:(numerator /. denominator :: acc_weights)
      in
      ...
      let rec calculate_weighted_throughput_mean ~curr_index ~acc =
        if curr_index >= window_size then
          ...
          (* the bandwidth in representation structure is measured in bits/s, so it should be here in the same type of units *)
          (* let measured_throughput = (result_.segment_size * 8 * us * us) / result_.time_for_delivery in *)
          (* OCaml's floating-point numbers follow the IEEE 754 standard, using double precision (64 bits) numbers *)
          let measured_throughput = ((float_of_int result_.segment_size) *. 8. *. us_float *. us_float) /. float_of_int (result_.time_for_delivery) in
          ...
          let () =
            if debug then begin
              ...
              print_endline @@ "result_.segment_number: " ^ string_of_int result_.segment_number;
              print_endline @@ "(List.nth_exn weights curr_index): " ^ string_of_float (List.nth_exn weights curr_index);
              print_endline @@ "measured_throughput = " ^ string_of_float measured_throughput;
            end
          in
          calculate_weighted_throughput_mean ~curr_index:(curr_index + 1) ~acc:(acc +. product)
      in
      let weighted_throughput_mean = calculate_weighted_throughput_mean ~curr_index:0 ~acc:0. in
      ...
      let rec calculate_throughput_variance ~curr_index ~acc =
        if curr_index >= window_size then
          ...
          let () =
            if debug then begin
              print_endline @@ "(result_.segment_size * 8 * us * us): " ^ string_of_int (result_.segment_size * 8 * us * us);
              print_endline @@ "result_.segment_size = " ^ string_of_int result_.segment_size;
              print_endline @@ "result_.time_for_delivery = " ^ string_of_int result_.time_for_delivery;
              print_endline @@ "measured_throughput = " ^ string_of_float measured_throughput;
            end
          in
          let next_sum = (List.nth_exn weights curr_index) *. ((measured_throughput -. weighted_throughput_mean) ** 2.) in
          calculate_throughput_variance ~curr_index:(curr_index + 1) ~acc:(acc +. next_sum)
      in
      let throughput_variance = calculate_throughput_variance ~curr_index:0 ~acc:0. in
      let variation_coefficient_theta = (sqrt throughput_variance) /.weighted_throughput_mean in
      (* bw_safety_factor is static parameter according to the paper *)
      let bw_safety_factor = 0.3 in
      let throughput_variance_scaling_factor = bw_safety_factor +. (1. -. bw_safety_factor) *. ((1. -. (min variation_coefficient_theta 1.)) ** 2.) in

      let buf_now =
        if (List.length results) = 0 then 0.
        else (List.hd_exn results).buffer_level_in_momentum
      in
      (* lower_buffer_bound, upper_buffer_bound are static parameters according to the paper *)
      let lower_buffer_bound, upper_buffer_bound = 0.5, 1.5 in
      let buffer_based_scaling_factor = lower_buffer_bound +. (upper_buffer_bound -. lower_buffer_bound) *. (buf_now /. algo.maxb) in

      let adaptive_throughput_estimate = weighted_throughput_mean *. throughput_variance_scaling_factor *. buffer_based_scaling_factor in
      let () =
        if debug then begin
          print_endline @@ "weighted_throughput_mean (the same unit as Del_Rate) = " ^ string_of_int @@ int_of_float @@ weighted_throughput_mean /. 1000.;
          print_endline @@ "throughput_variance_scaling_factor = " ^ string_of_float throughput_variance_scaling_factor;
          print_endline @@ "buffer_based_scaling_factor = " ^ string_of_float buffer_based_scaling_factor;
          print_endline @@ "adaptive_throughput_estimate (the same unit as Del_Rate) = " ^ string_of_int @@ int_of_float @@ adaptive_throughput_estimate /. 1000.;
        end
      in
      ...
      let s_rate = Hashtbl.fold representations ~init:rate_min ~f:(fun ~key ~data acc ->
          ...
          in
          if data.bandwidth > acc && data.bandwidth < int_of_float adaptive_throughput_estimate then
            data.bandwidth
          else
            acc) in
      ...
      let rec calculate_actual_rate ~next_repr_candidate ~curr_index ~acc =
        if curr_index > look_ahead_window then
          ...
          let next_segment_size = List.nth_exn (Hashtbl.find_exn algo.chunk_sizes_per_repr next_repr_candidate) (segm_number_prev + curr_index) in
          calculate_actual_rate ~next_repr_candidate:next_repr_candidate ~curr_index:(curr_index + 1) ~acc:(acc + next_segment_size)
      in
      let rec highest_possible_actual_rate ~next_repr_candidate =
        let actual_rate = calculate_actual_rate ~next_repr_candidate:next_repr_candidate ~curr_index:1 ~acc:0 in
        let () =
          if debug then begin
        ...
        if next_repr_candidate > 1 && not (actual_rate <= int_of_float adaptive_throughput_estimate) then
          highest_possible_actual_rate ~next_repr_candidate:(next_repr_candidate - 1)
        else
          next_repr_candidate
      in
      let actual_next_rate = highest_possible_actual_rate ~next_repr_candidate:next_repr in
      actual_next_rate
end

...
let request ?conn ?interrupt ?ssl_config ?uri ?(body=`Empty) req =
...
let call ?conn ?interrupt ?ssl_config ?headers ?(chunked=false) ?(body=`Empty) meth uri =
  ...
  in
  req >>= request ?conn ?interrupt ?ssl_config ~body ~uri

let get ?conn ?interrupt ?ssl_config ?headers uri =
  call ?conn ?interrupt ?ssl_config ?headers ~chunked:false `GET uri

let head ?conn ?interrupt ?ssl_config ?headers uri =
  call ?conn ?interrupt ?ssl_config ?headers ~chunked:false `HEAD uri
  >>| fun (res, body) ->
  (match body with
